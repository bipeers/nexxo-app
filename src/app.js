const express = require('express');
const cors = require('cors');
const helmet = require('helmet');
const mongoSanitize = require('express-mongo-sanitize');
const rateLimit = require('express-rate-limit');
const hpp = require('hpp');
const dotenv = require('dotenv');
const { body, validationResult } = require('express-validator');
const { SessionsClient } = require('@google-cloud/dialogflow-cx');

// Load env vars
dotenv.config();

const app = express();

// Initialize Dialogflow CX Sessions Client
// Note: It will automatically use credentials from GOOGLE_APPLICATION_CREDENTIALS if set
const aiSessionClient = new SessionsClient();

// Security Middlewares
// 1. Set Security HTTP headers
app.use(helmet());

// 2. Rate limiting
// Limit each IP to 100 requests per windowMs (10 minutes)
const limiter = rateLimit({
  windowMs: 10 * 60 * 1000,
  max: 100,
  message: 'Too many requests from this IP, please try again after 10 minutes'
});
app.use('/api', limiter);

// 3. Body parser, reading data from body into req.body
app.use(express.json({ limit: '10kb' }));
app.use(express.urlencoded({ extended: true, limit: '10kb' }));

// 4. Data sanitization against NoSQL query injection
app.use(mongoSanitize());

// 5. Prevent parameter pollution
app.use(hpp());

// 6. Enable CORS
// Configure this according to your specific frontend URL in production
const corsOptions = {
    origin: process.env.NODE_ENV === 'production'
        ? process.env.FRONTEND_URL
        : '*',
    credentials: true
};
app.use(cors(corsOptions));

// Routes
app.get('/', (req, res) => {
    res.status(200).json({ status: 'success', message: 'Welcome to Nexxo API' });
});

app.get('/api/health', (req, res) => {
    res.status(200).json({ status: 'success', message: 'Server is healthy' });
});

// AI features route using Google Cloud Dialogflow CX
app.post('/api/ai/chat',
    [
        body('prompt')
            .notEmpty().withMessage('Prompt is required')
            .isString().withMessage('Prompt must be a string')
            .trim(),
        body('sessionId')
            .notEmpty().withMessage('Session ID is required')
            .isString().withMessage('Session ID must be a string')
            .trim()
            .escape()
    ],
    async (req, res, next) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        }

        const { prompt, sessionId } = req.body;

        // Agent ID and location need to be configured via environment variables
        const agentId = process.env.DIALOGFLOW_AGENT_ID;
        const location = process.env.DIALOGFLOW_LOCATION || 'global';
        const projectId = process.env.PROJECT_ID || process.env.GOOGLE_CLOUD_PROJECT;

        if (!agentId || !projectId) {
            return res.status(503).json({
                status: 'error',
                message: 'Dialogflow agent is not configured properly. Missing PROJECT_ID or DIALOGFLOW_AGENT_ID.'
            });
        }

        try {
            const sessionPath = aiSessionClient.projectLocationAgentSessionPath(
                projectId,
                location,
                agentId,
                sessionId
            );

            const request = {
                session: sessionPath,
                queryInput: {
                    text: {
                        text: prompt,
                    },
                    languageCode: 'en', // You might want to make this configurable
                },
            };

            const [response] = await aiSessionClient.detectIntent(request);

            // Extract the text response if available
            let replyText = '';
            if (response.queryResult && response.queryResult.responseMessages) {
                for (const message of response.queryResult.responseMessages) {
                    if (message.text) {
                        replyText += message.text.text.join(' ') + ' ';
                    }
                }
            }

            res.status(200).json({
                status: 'success',
                response: replyText.trim() || 'No response text received.',
                intent: response.queryResult?.intent?.displayName || 'Unknown Intent'
            });
        } catch (error) {
            console.error('Dialogflow Error:', error);
            next(error);
        }
    }
);

// Example route with explicit input validation
app.post('/api/data',
    // Validation chain - targeted validation and sanitization
    [
        body('data')
            .notEmpty().withMessage('Data is required')
            .isString().withMessage('Data must be a string')
            .trim()
            .escape() // HTML escaping as an extra layer
    ],
    (req, res) => {
        // Check validation results
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        }

        const { data } = req.body;

        // Process data...
        res.status(200).json({ status: 'success', received: data });
    }
);

// Catch-all route for undefined routes
app.use((req, res, next) => {
    res.status(404).json({
        status: 'fail',
        message: `Can't find ${req.originalUrl} on this server!`
    });
});

// Error handling middleware
app.use((err, req, res, next) => {
    console.error(err.stack);
    res.status(err.status || 500).json({
        status: 'error',
        message: err.message || 'Internal server error'
    });
});

module.exports = app;
